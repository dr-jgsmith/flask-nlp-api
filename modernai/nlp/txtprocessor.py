...
class TxtProcessor:
    ...
    def atomize(self, text):
        ...
        # Atomize and tokens are optional standalone tools. They can be used with or without the Spacy.io framework.
        :param text: string
        :return: list
        """
    ...
    def tokenize(self):
        ...
        :return: clean list of tokens
        """
        self.tokens = [word.text for word in self.data if word.is_stop is False]
        return self.tokens

    def rank_tokens(self):
        token_ranks = [(word.rank, word.text, word.i, word.orth) for word in self.data if word.is_stop is False]
        return token_ranks
    ...
    def custom_corpora(self, token_set, path_to_dict, path_to_model):
        if os.path.exists(path_to_dict):
            dictionary = corpora.Dictionary.load(path_to_dict)
            corpus = corpora.MmCorpus(path_to_model)
        else:
            dictionary = corpora.Dictionary(token_set[0])
            dictionary.save(path_to_dict)
            corpus = [dictionary.doc2bow(tokens) for tokens in token_set[0]]
            corpora.MmCorpus.serialize(path_to_model, corpus)
